...
public class AggregatePropertiesTest {
    ...
    public void testSum() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                ...
                IObservableValue sumValue = AggregateProperties
                        .sum(EMFProperties
                                .list(ClassroomPackage.Literals.STUDENT__TEST_RESULTS)
    ...
    public void testAverage() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                Student studentToObserve = studentList.get(0);
                IObservableValue avgValue = AggregateProperties
                        .average(
                                EMFProperties
                                        .list(ClassroomPackage.Literals.STUDENT__TEST_RESULTS)
                                        .values(ClassroomPackage.Literals.EXAM_RESULT__SCORE))
    ...
    public void testNth() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                Student studentToObserve = studentList.get(0);
                IObservableValue zero = AggregateProperties
                        .nth(0,
                                EMFProperties
                                        .list(ClassroomPackage.Literals.STUDENT__TEST_RESULTS))
                        .observe(studentToObserve);

                IObservableValue four = AggregateProperties
                        .nth(4,
                                EMFProperties
                                        .list(ClassroomPackage.Literals.STUDENT__TEST_RESULTS))
                        .observe(studentToObserve);

                assertSame(studentToObserve.getTestResults().get(0),
                        zero.getValue());
                assertSame(studentToObserve.getTestResults().get(4),
                        four.getValue());
            }
        });
    }
    ...
    @Test
    public void testNthBuried() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                List<StudentBean> students = new ArrayList<StudentBean>();
                students.add(createStudentBean("Bill"));
                students.add(createStudentBean("Bob"));
                students.add(createStudentBean("Susan"));
                students.add(createStudentBean("Steve"));
                students.add(createStudentBean("Tommy"));
                List<Wrapper> wrappers = new ArrayList<Wrapper>();
                wrappers.add(new Wrapper(students));
                wrappers.add(new Wrapper(new ArrayList<Object>()));
                IValueProperty firstStudentsName = AggregateProperties.nth(0,
                        BeanProperties.list("context", StudentBean.class))
                        .value(BeanProperties.value("name"));
                IValueProperty secondStudentsName = AggregateProperties.nth(1,
                        BeanProperties.list("context")).value(
                        BeanProperties.value("name"));
                IValueProperty thirdStudentsName = AggregateProperties.nth(2,
                        BeanProperties.list("context")).value(
                        BeanProperties.value("name"));
                IValueProperty fourthStudentsName = AggregateProperties.nth(3,
                        BeanProperties.list("context")).value(
                        BeanProperties.value("name"));
                IValueProperty fifthStudentsName = AggregateProperties.nth(4,
                        BeanProperties.list("context")).value(
                        BeanProperties.value("name"));
                IObservableList observableBeanList = new WritableList(wrappers,
                        Wrapper.class);
                IObservableSet observableBeanSet = new ListToSetAdapter(
                        observableBeanList);
                IObservableMap firstStudentNameMap = firstStudentsName
                        .observeDetail(observableBeanSet);
                IObservableMap secondStudentNameMap = secondStudentsName
                        .observeDetail(observableBeanSet);
                IObservableMap thirdStudentNameMap = thirdStudentsName
                        .observeDetail(observableBeanSet);
                IObservableMap fourthStudentNameMap = fourthStudentsName
                        .observeDetail(observableBeanSet);
                IObservableMap fifthStudentNameMap = fifthStudentsName
                        .observeDetail(observableBeanSet);
                assertEquals("Bill", firstStudentNameMap.get(wrappers.get(0)));
                assertEquals("Bob", secondStudentNameMap.get(wrappers.get(0)));
                assertEquals("Susan", thirdStudentNameMap.get(wrappers.get(0)));
                assertEquals("Steve", fourthStudentNameMap.get(wrappers.get(0)));
                assertEquals("Tommy", fifthStudentNameMap.get(wrappers.get(0)));
                assertNull(firstStudentNameMap.get(wrappers.get(1)));
            }
        });
    }
    ...
    public void testWeightedAverageCalc() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                ...
                // average.
                double manualAvg = calcWeightedAvg(studentToObserve
                        .getTestResults());
    ...
    public void testMinAndMax() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                Student studentToObserve = studentList.get(0);
                IListProperty allResults = EMFProperties
                        .list(ClassroomPackage.Literals.STUDENT__TEST_RESULTS);
                // We're going to be using the fact that TestResults are
                // Comparable to get a min and max ExamResult.
                ...
                assertEquals(studentToObserve.getTestResults().get(0),
                        minValue.getValue());
                assertEquals(studentToObserve.getTestResults().get(4),
                        maxValue.getValue());
            }
        });
    }
    ...
    public void testObservableList() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                ...
                IObservableList observableBeanList = new WritableList(
                        studentList, Student.class);
                // This next list should be the sum of each individual student's
                // scores.
                IObservableList list = AggregateProperties
                        .sum(EMFProperties
                                .list(ClassroomPackage.Literals.STUDENT__TEST_RESULTS)
                                .values(ClassroomPackage.Literals.EXAM_RESULT__SCORE))
                        .observeDetail(observableBeanList);

                assertEquals(calcSum(studentList.get(0).getTestResults()),
                        (Double) list.get(0), 0.0d);
                assertEquals(calcSum(studentList.get(1).getTestResults()),
                        (Double) list.get(1), 0.0d);
                assertEquals(calcSum(studentList.get(2).getTestResults()),
                        (Double) list.get(2), 0.0d);
                assertEquals(calcSum(studentList.get(3).getTestResults()),
                        (Double) list.get(3), 0.0d);
                assertEquals(calcSum(studentList.get(4).getTestResults()),
                        (Double) list.get(4), 0.0d);
                studentList.get(0).getTestResults().clear();
                studentList.get(0).getTestResults()
                        .add(ClassroomFactory.eINSTANCE.createExamResult(1, 1));
                studentList.get(0).getTestResults()
                        .add(ClassroomFactory.eINSTANCE.createExamResult(1, 1));
    ...
    public void testObservableMap() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                ...
                IObservableSet observableBeanSet = new ListToSetAdapter(
                        observableBeanList);
                IObservableMap map = AggregateProperties
                        .sum(EMFProperties
                                .list(ClassroomPackage.Literals.STUDENT__TEST_RESULTS)
                                .values(ClassroomPackage.Literals.EXAM_RESULT__SCORE))
                        .observeDetail(observableBeanSet);
                ...
                assertEquals(calcSum(studentList.get(0).getTestResults()),
                        (Double) map.get(studentList.get(0)), 0.0d);
                assertEquals(calcSum(studentList.get(1).getTestResults()),
                        (Double) map.get(studentList.get(1)), 0.0d);
                assertEquals(calcSum(studentList.get(2).getTestResults()),
                        (Double) map.get(studentList.get(2)), 0.0d);
                assertEquals(calcSum(studentList.get(3).getTestResults()),
                        (Double) map.get(studentList.get(3)), 0.0d);
                assertEquals(calcSum(studentList.get(4).getTestResults()),
                        (Double) map.get(studentList.get(4)), 0.0d);
                studentList.get(0).getTestResults().clear();
                studentList.get(0).getTestResults()
                        .add(ClassroomFactory.eINSTANCE.createExamResult(1, 1));
                studentList.get(0).getTestResults()
    ...
    @Test
    public void testObservableMapWithoutListener() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                List<Student> newStudentList = new ArrayList<Student>();
                WritableList observableBeanList = new WritableList(
                        newStudentList, Student.class);
                IObservableSet observableBeanSet = new ListToSetAdapter(
                        observableBeanList);
                IObservableMap map = AggregateProperties
                        .sum(EMFProperties
                                ...
                                .values(ClassroomPackage.Literals.EXAM_RESULT__SCORE))
                        .observeDetail(observableBeanSet);
                assertNull(map.get(studentList.get(0)));
                observableBeanList.addAll(studentList);
                assertEquals(calcSum(studentList.get(0).getTestResults()),
                        map.get(studentList.get(0)));
            }
        });
    }
    ...
    private StudentBean createStudentBean(String name) {
    ...
    private static class StudentBean {
        private String name;
