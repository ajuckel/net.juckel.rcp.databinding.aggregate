...
public class AggregatePropertiesTest {
        ...
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                ...
                IObservableValue value =
                        AggregateProperties.sum(
                            Properties.selfList(Bean.class)
                                .values(BeanProperties.value("value", Bean.class)))
                            .observe(beanList);
    ...
    public void testAverage() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                IObservableValue value =
                        AggregateProperties.average(
                            Properties.selfList(Bean.class)
                                .values(BeanProperties.value("value", Bean.class)))
                            .observe(beanList);
                assertEquals(3.0d, ((Double) value.getValue()).doubleValue(), 0.0d);
                Bean b = beanList.get(0);
                b.setValue(5.0);
                assertEquals(3.8, ((Double) value.getValue()).doubleValue(), 0.0d);
            }
        });
    }
    ...
    public void testNth() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                IObservableValue zero =
                        AggregateProperties.nth(0,
                            Properties.selfList(Bean.class))
                            .observe(beanList);
    ...
    public void testListening() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                ...
                Bean b = new Bean(1.0);
                IObservableValue value =
                        AggregateProperties.sum(BeanProperties.list("values", Bean.class))
                            .observe(b);
    ...
    public void testWeightedAverageCalc() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                IObservableValue observableWeightedAverage =
                        AggregateProperties.weightedAverage(
                            Properties.selfList(Bean.class).values(BeanProperties.value("value")),
                            Properties.selfList(Bean.class).values(BeanProperties.value("weight")))
                            .observe(beanList);

                // Beans default to a weight of 1, so we should have a simple
                // average.
                assertEquals(3.0d, ((Double) observableWeightedAverage.getValue()).doubleValue(),
                    0.0d);
                ...
                assertEquals(3.888889,
                    ((Double) observableWeightedAverage.getValue()).doubleValue(), 0.000001d);
                beanList.get(3).setWeight(4.0);
                beanList.get(2).setWeight(3.0);
                beanList.get(1).setWeight(2.0);

                assertEquals(3.666667,
                    ((Double) observableWeightedAverage.getValue()).doubleValue(), 0.000001d);
            }
        });
    }
    ...
    public void testMinAndMax() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                IObservableValue minValue = AggregateProperties.min(Properties.selfList(Bean.class))
                    .observe(beanList);
                IObservableValue maxValue = AggregateProperties.max(Properties.selfList(Bean.class))
                    .observe(beanList);
    ...
    public void testObservableList() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                IObservableList observableBeanList = new WritableList(beanList, Bean.class);
                IObservableList list =
                        AggregateProperties.sum(BeanProperties.list("values", Integer.class))
    ...
    public void testObservableMap() {
        Realm.runWithDefault(realm, new Runnable() {
            public void run() {
                IObservableList observableBeanList = new WritableList(beanList, Bean.class);
                IObservableSet observableBeanSet = new ListToSetAdapter(observableBeanList);
                IObservableMap map =
                        AggregateProperties.sum(BeanProperties.list("values", Integer.class))
    ...
    public class Bean implements Comparable<Bean> {
        ...
        public void setValues(Integer[] values) {
            ...
            this.pcs.fireIndexedPropertyChange("values", 0, oldValue, this.getValues());
        }
        ...
        public void setValues(int index, Integer value) {
            ...
            this.pcs.fireIndexedPropertyChange("values", index, oldValue, this.getValues());
        }
        ...
        public int compareTo(Bean o) {
            if (o == null) {
                ...
                return Double.compare(this.value * this.weight, o.getValue() * o.getWeight());
            }
        }
    }
}
